...
### 풀이 방법
1. 각 응답 완료 시각 기준으로 1초 구간을 잡아 처리량을 계산한다.  
  1-1. 응답 완료시각이 이미 오름차순으로 정렬되어 있음  
  1-2. 다음 응답 완료시각의 시작 시간이 이 구간에 겹치면 반드시 처리 건수에 포함됨
2. 모든 완료 시점마다 1초 구간을 검사하고 가장 높은 처리량을 찾아 반환한다.


### 회고
1. 처음에는 단순하게 하루니깐 1ms마다 윈도우를 두고 전부 확인하려 했다.
2. 파라메트릭 서치를 구상했으나 범위를 올리고 내릴 기준이 없어 아닌 것으로 판단했다.
3. 그렇게 되면 시간 복잡도가 어마어마해졌다. 대략 1700억번의 계산이 필요했는데 역시나 실행 시간이 초과 됐다.
4. 그래서 로그 기준으로 로그들의 시작 시간부터 윈도우를 옮겨나가려고 했는데 이것도 약 120억번의 연산이 필요해서 실패했다.
5. 그러다가 임의의 순서를 부여하는 방법이 기억났다.
6. 자세히보니 종료시각 기준으로 1초 구간에 포함되는 요청은 처리량에 포함할 수 있었다.
7. 이 연산은 n^2으로 가능했고 실제로 통했다.
8. 다른 사람의 풀이를 보니 투포인터를 사용하여 O(nlogn)로 개선시킬 수 있었다.
9. 그러고 보면 이전에 풀어봤던 문제도 이런 식으로 개선했었는데... 이게 기억이 안났네 ㅠ
10. ms단위로 계산해야하지 않을까라는 생각때문에 풀이에 오래 걸렸다.
11. 단순 풀이를 진행했을 때 실행시간이 긴 것을 빠르게 캐치하고 이런 아이디어를 구상했어야했다.
12. 풀이 과정은 좋았으나 빠르게 판단할 필요가 있다.